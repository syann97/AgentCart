...
- 현재 Agent 경로의 `searchCatalog`는 MySQL에서 상태·재고·명시 가격·명시 카테고리·최근 주문 조건으로 허용 ID를 먼저 제한하고, 검색 결과를 최신 상품 기준으로 다시 검증합니다. 비교 기준인 기존 `RecommendationService`는 후보 검색 이후에 정책을 검증합니다.
...
LLM 장애 처리에서 사용자 조건을 없애지 않습니다. 현재 Agent fallback도 같은 요청 컨텍스트로 `searchCatalog`를 호출해 명시 조건과 서버 정책을 유지합니다. 비교 기준인 기존 `RecommendationService`의 질의 확장 fallback은 원문 질의와 빈 추론 조건으로 돌아가므로 두 경로를 구분합니다. Future timeout과 실제 외부 호출 취소를 구분합니다.
