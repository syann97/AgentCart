...
문서 정합성 정비와 에이전트 기반 코드는 단계별로 진행합니다. Agent와 SSE 계약은 구현됐으며 실제 모델 품질 평가는 provider credit을 준비한 뒤 고정 snapshot에서 재실행해야 합니다. 현재/목표 구분은 [현재 파이프라인](RECOMMENDATION_PIPELINE.md)과 [구현 계획](AGENTIC_RAG_PLAN.md)을 기준으로 확인합니다.
