...
function renderRow(props: Partial<Parameters<typeof CartItemRow>[0]> = {}) {
  return render(
    <CartItemRow
      item={activeItem}
      checked={false}
...
describe('CartItemRow', () => {
  ...
  it('SOLD_OUT 상품은 수량 버튼과 입력 필드가 비활성화된다', () => {
    renderRow({ item: { ...activeItem, productStatus: 'SOLD_OUT' } });
  ...
  it('수량 1에서 [-] 클릭 시 삭제 확인 다이얼로그가 표시된다', async () => {
    renderRow({ item: { ...activeItem, quantity: 1 } });
    await userEvent.click(screen.getByLabelText('수량 감소'));
  ...
  it('재고 초과 입력 시 에러 메시지가 표시되고 재고 최대값으로 클램프된다', () => {
    renderRow({ item: { ...activeItem, productStock: 3 } });
  ...
  it('주문 상한(10개) 초과 시 에러 메시지가 표시되고 10으로 클램프된다', async () => {
    renderRow({ item: { ...activeItem, productStock: 20, quantity: 10 } });
    await userEvent.click(screen.getByLabelText('수량 증가'));
  ...
  it('0 이하 입력 시 에러 메시지가 표시되고 1로 클램프된다', () => {
    renderRow();
  ...
  it('checked=false 이면 체크박스가 선택되지 않은 상태로 렌더링된다', () => {
    renderRow({ checked: false });
    expect((screen.getByLabelText('상품 선택') as HTMLInputElement).checked).toBe(false);
  });

  it('checked=true 이면 체크박스가 선택된 상태로 렌더링된다', () => {
    renderRow({ checked: true });
    expect((screen.getByLabelText('상품 선택') as HTMLInputElement).checked).toBe(true);
  });

  it('체크박스 클릭 시 onCheckedChange가 true로 호출된다', async () => {
    renderRow({ checked: false });
    await userEvent.click(screen.getByLabelText('상품 선택'));
    expect(mockOnCheckedChange).toHaveBeenCalledWith(true);
  });

  it('SOLD_OUT 상품의 체크박스는 비활성화된다', () => {
    renderRow({ item: { ...activeItem, productStatus: 'SOLD_OUT' } });
    expect(screen.getByLabelText('상품 선택')).toBeDisabled();
  });
});